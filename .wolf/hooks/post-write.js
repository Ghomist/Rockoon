...
async function main() {
  ...
  try {
    ...
    const relPathLocal = normalizePath(
    ...
    const tmp =
      anatomyPath + "." + crypto.randomBytes(4).toString("hex") + ".tmp";
    try {
...
function summarizeEdit(oldStr, newStr, filename) {
  ...
  if (
    newStr.includes("try") &&
    newStr.includes("catch") &&
    !oldStr.includes("catch")
  ) {
  ...
  // --- Import changes ---
  const oldImports = oldLines.filter(l =>
    /^\s*(import|require|use |from )/.test(l)
  ).length;
  const newImports = newLines.filter(l =>
    /^\s*(import|require|use |from )/.test(l)
  ).length;
  if (
    newImports > oldImports &&
    Math.abs(newCount - oldCount) <= newImports - oldImports + 1
  ) {
  ...
  if (
    ...
    const oldProps = (oldStr.match(/[\w-]+\s*:/g) || []).map(p =>
      p.replace(/\s*:/, "")
    );
    const newProps = (newStr.match(/[\w-]+\s*:/g) || []).map(p =>
      p.replace(/\s*:/, "")
    );
  ...
  // --- Function modified ---
  const fnMatch = newStr.match(
    /(?:function|def|fn|func|async\s+function)\s+(\w+)/
  );
  if (fnMatch) {
...
function detectFixPattern(oldStr, newStr, ext) {
  ...
  if (newStr.includes("catch") && !oldStr.includes("catch")) {
    ...
    return {
      ...
      rootCause:
        "Code path had no error handling — exceptions would propagate uncaught",
  ...
  if (
    ...
    (newStr.includes("?? ") && !oldStr.includes("?? ")) ||
    (/!==?\s*(null|undefined)/.test(newStr) &&
      !/!==?\s*(null|undefined)/.test(oldStr))
  ) {
    return {
  ...
  const oldImports = new Set(
    (oldStr.match(/(?:import|require)\s*\(?['"]([^'"]+)['"]\)?/g) || []).map(
      m => m
    )
  );
  const newImports =
    newStr.match(/(?:import|require)\s*\(?['"]([^'"]+)['"]\)?/g) || [];
  const addedImports = newImports.filter(i => !oldImports.has(i));
  if (
    addedImports.length > 0 &&
    newLines.length - oldLines.length <= addedImports.length + 2
  ) {
    const modules = addedImports
      .map(i => i.match(/['"]([^'"]+)['"]/)?.[1] || "")
      .filter(Boolean);
    return {
  ...
  if (
    ...
    if (changedProps.length > 0 && changedProps.length <= 3) {
      const desc = changedProps
        .map(([k, v]) => `${k}: ${oldProps.get(k)} → ${v}`)
        .join("; ");
      return {
        category: "style-fix",
  ...
  const diffRatio =
    Math.abs(newStr.length - oldStr.length) / Math.max(oldStr.length, 1);
